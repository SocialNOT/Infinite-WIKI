...
/**
 * Streams a translation for a given text and language.
 * @param text The text to translate.
 * @param language The target language.
 * @returns An async generator that yields translated text chunks.
 */
export async function* streamTranslation(
  ...
  const prompt = `Translate the following text into ${language}. Do not add any extra commentary, titles, or formatting. Provide only the raw translated text. The text to translate is:\n\n"${text}"`;

  try {
    const response = await ai.models.generateContentStream({
      model: textModelName, // Use the fast model
      contents: prompt,
      config: {
        thinkingConfig: { thinkingBudget: 0 },
      },
    ...
    const errorMessage =
      error instanceof Error ? error.message : 'An unknown error occurred.';
    yield `Error: Could not translate content. ${errorMessage}`;
    throw new Error(errorMessage);
  }
}
